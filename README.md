...
>
> **仓库地址：** https://github.com/Hap-hub/mining-evaluation-skills
>
> ## 📬 Contact Me

- WeChat: **happyhey**
- Email: cnheyong@email.com
- Linkedin: https://www.linkedin.com/in/heyonghy
- GitHub: https://github.com/hap-hub
...
- [四、国产大模型配置方法](#四国产大模型配置方法)
- [五、在 Kimi Code 中安装](#五在-kimi-code-中安装)
- [使用示例](#使用示例)
- [常见问题](#常见问题)
...
git clone https://github.com/Hap-hub/mining-evaluation-skills.git

# 方式 2：下载 zip 后解压
#   打开 https://github.com/Hap-hub/mining-evaluation-skills
...
如果你希望在国内网络环境或国产模型服务上使用本 Skill，推荐采用 **OpenClaw + OpenAI Compatible API** 的路径：OpenClaw 作为模型无关的宿主，底层模型配置为 DeepSeek 或 Z.ai（GLM）。Skill 仍按上文方式放在 `.claude/skills/mining-evaluation-skills/` 中，模型只负责执行和生成。

### 方式 A：OpenClaw + DeepSeek

1. 在 DeepSeek 控制台创建 API Key。
2. 在 OpenClaw 的模型设置中新增或选择 **OpenAI Compatible** 类型。
3. 填入以下配置：
   ...
   Model: deepseek-chat（或你在 DeepSeek 控制台已启用的模型）
   ```

4. 保存后重新开启 OpenClaw 会话，并用一个矿业评估问题测试 Skill 是否自动触发。

### 方式 B：OpenClaw + Z.ai（GLM）

1. 在 Z.ai 控制台创建 API Key，并确认你使用的是普通 API 还是 GLM Coding Plan。
2. 在 OpenClaw 的模型设置中新增或选择 **OpenAI Compatible** 类型。
3. 按你的账户类型填入端点：

   ```text
   Provider / Type: OpenAI Compatible
   Base URL: https://api.z.ai/api/paas/v4/              # 普通 API
   Base URL: https://api.z.ai/api/coding/paas/v4        # GLM Coding Plan
   API Key: 你的 Z.ai API Key
   Model: 你的 GLM 模型代码，例如 glm-4.5 或 glm-5.2
   ```

4. 如果 OpenClaw 提供上下文窗口设置，按所选 GLM 模型的官方说明填写；如果出现 401 / 403，优先检查 API Key 类型、订阅计划和 Base URL 是否匹配。

> 提示：国产模型配置只改变底层推理模型，不改变 Skill 的安装位置。若 Skill 未触发，先检查 `.claude/skills/mining-evaluation-skills/SKILL.md` 是否存在，再重启 OpenClaw / Claudian 会话。

---

## 五、在 Kimi Code 中安装

Kimi Code CLI 支持 Agent Skills。它加载的是包含 `SKILL.md` 的目录，而不是直接上传 Claude.ai 使用的 `.skill` 包；如果你从 Release 下载的是 `mining-evaluation-skills.skill`，需要先解包成目录再放入 Kimi Code 的 skills 扫描路径。

### 方式 A：直接克隆仓库（推荐）

用户级安装，对所有 Kimi Code 项目生效：

```bash
mkdir -p ~/.kimi-code/skills
git clone https://github.com/Hap-hub/mining-evaluation-skills.git \
  ~/.kimi-code/skills/mining-evaluation-skills
```
...
重启 Kimi Code 会话后，直接提出矿业评估问题即可自动触发；也可以手动调用：

```bash
/skill:mining-evaluation-skills 评估这个铜矿项目的资源量分类风险
```

### 方式 B：下载并加载 `.skill` 文件

1. 打开仓库 Release 页面：<https://github.com/Hap-hub/mining-evaluation-skills/releases>。
2. 下载最新版附件 `mining-evaluation-skills.skill`。
3. 将 `.skill` 当作 zip 包解压，确保解压后的目录根部存在 `SKILL.md`：

   ```bash
   mkdir -p ~/.kimi-code/skills/mining-evaluation-skills
   unzip mining-evaluation-skills.skill -d ~/.kimi-code/skills/mining-evaluation-skills
   test -f ~/.kimi-code/skills/mining-evaluation-skills/SKILL.md
   ```

   Windows PowerShell 可先复制为 `.zip` 再解压：

   ```powershell
   New-Item -ItemType Directory -Force "$env:USERPROFILE\.kimi-code\skills\mining-evaluation-skills"
   Copy-Item .\mining-evaluation-skills.skill .\mining-evaluation-skills.zip
   Expand-Archive .\mining-evaluation-skills.zip "$env:USERPROFILE\.kimi-code\skills\mining-evaluation-skills" -Force
   Test-Path "$env:USERPROFILE\.kimi-code\skills\mining-evaluation-skills\SKILL.md"
   ```

4. 重启 Kimi Code，会话启动后它会扫描 `~/.kimi-code/skills/` 并发现该 Skill。

> 提示：Kimi Code 也会扫描 `~/.agents/skills/`、项目级 `.agents/skills/` 和 `.kimi-code/skills/`。如果你希望与 Codex 共用同一份 Skill，可以把仓库放在 `~/.agents/skills/mining-evaluation-skills/`。
...
**仓库：** https://github.com/Hap-hub/mining-evaluation-skills · 欢迎提 Issue / Fork / Star ⭐
